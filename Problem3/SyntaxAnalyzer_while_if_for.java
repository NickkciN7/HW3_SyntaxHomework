...
public class SyntaxAnalyzer_while_if_for {
	static LexicalAnalyzer lexAnalyzer;
	
	public static void main(String[] args) {
		
//		lexAnalyzer = new LexicalAnalyzer("TEST_Problem3_while.txt");
//		while_statement();
//		lexAnalyzer.closeFile();
		
//		lexAnalyzer = new LexicalAnalyzer("TEST_Problem3_if.txt");
//		selection_statement();
//		lexAnalyzer.closeFile();
		
		lexAnalyzer = new LexicalAnalyzer("TEST_Problem3_for.txt");
		for_statement();
		lexAnalyzer.closeFile();
	...
	public static void while_statement() {
		...
		if(lexAnalyzer.nextToken != lexAnalyzer.WHILE_CODE) {
			...
			if(lexAnalyzer.nextToken != lexAnalyzer.LEFT_PAREN) {
				...
				lexAnalyzer.lex();
				expression(""); //just a normal expression type so pass empty string
				if(lexAnalyzer.nextToken != lexAnalyzer.RIGHT_PAREN) {
					error("WHILE");
				}
				else {
					lexAnalyzer.lex();
					statement("WHILE"); //so statement can pass "WHILE" to error() in case of error
				}
			}
		}
	...
	public static void selection_statement() {
		System.out.println("Enter <selection_statement>");
		
		if(lexAnalyzer.nextToken != lexAnalyzer.IF_CODE) {
			error("IF");
		}
		else {
			lexAnalyzer.lex();
			if(lexAnalyzer.nextToken != lexAnalyzer.LEFT_PAREN) {
				...
				if(lexAnalyzer.nextToken != lexAnalyzer.RIGHT_PAREN) {
					...
					lexAnalyzer.lex();
					statement("IF");
					if(lexAnalyzer.nextToken == lexAnalyzer.ELSE_CODE) {
						lexAnalyzer.lex();
						statement("IF"); //so statement can pass "IF" to error() in case of error
					}
				}
			}
		}
		...
		System.out.println("Exit <selection_statement>");
	}	
	...
	public static void for_statement() {
		...
		if(lexAnalyzer.nextToken != lexAnalyzer.FOR_CODE) {
			error("FOR");
		}
		else {
			lexAnalyzer.lex();
	...
	//start expression related methods
	public static void expression(String expType) {
		System.out.println("Enter <expression>");
		
		//not actually checking if a valid c/java like expression, just getting tokens until right parenthesis or end of file is found
		while(lexAnalyzer.nextToken != lexAnalyzer.EOF & lexAnalyzer.nextToken != lexAnalyzer.RIGHT_PAREN) {
			lexAnalyzer.lex();
		}
		
		System.out.println("Exit <expression>");
	}
	//difference with method above is that below checks for ; in while loop rather than )
	public static void expressionFor(String expType) {
		System.out.println("Enter <"+expType+"expression>");
		
		//not actually checking if a valid c/java like expression, just getting tokens until right parenthesis or end of file is found
		while(lexAnalyzer.nextToken != lexAnalyzer.EOF & lexAnalyzer.nextToken != lexAnalyzer.SEMICOLON) {
			lexAnalyzer.lex();
		}
		
		System.out.println("Exit <"+expType+"expression>");
	}
	//just call expressionFor() but pass the type so expression says proper name in print statements
	public static void init_expression(){
		expressionFor("init_");
	}
	public static void cond_expression(){
		expressionFor("cond_");
	}
	public static void loop_expression(){
		expressionFor("loop_");
	}
	//end expression related methods
	
	
	public static void statement(String statementType) {
		System.out.println("Enter <statement>");
		
		//not actually checking if a valid c/java like statement, just getting tokens starting with a  '{' until a '}', or end of file found
		if(lexAnalyzer.nextToken != lexAnalyzer.LEFT_CURLY) {
			error(statementType);
		}
		else {
			lexAnalyzer.lex();
			while(lexAnalyzer.nextToken != lexAnalyzer.EOF & 
				  lexAnalyzer.nextToken != lexAnalyzer.RIGHT_CURLY) 
			{
				lexAnalyzer.lex();
			}
			lexAnalyzer.lex();
	...
	public static void error(String statementType) {
		System.out.println("\nERROR, NOT A VALID " + statementType + " STATEMENT\n");
	}
}
...
class LexicalAnalyzer {
	...
	final static int SEMICOLON = 6;
	final static int LEFT_CURLY = 5;
	final static int RIGHT_CURLY = 4;
		//reserved words
